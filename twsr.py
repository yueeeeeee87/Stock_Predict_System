...
def get_tw_stock(stockNo, date):
    ...
    df['日期'] = df['日期'].apply(convert_date)
    df['Volume'] = pd.to_numeric(df['成交股數'].str.replace(",",""))
    df['成交金額'] = pd.to_numeric(df['成交金額'].str.replace(",",""))
    df['成交筆數'] = pd.to_numeric(df['成交筆數'].str.replace(",",""))
    df['Open'] = pd.to_numeric(df['開盤價'].str.replace(",",""))
    df['High'] = pd.to_numeric(df['最高價'].str.replace(",",""))
    df['Low'] = pd.to_numeric(df['最低價'].str.replace(",",""))
    df['Close'] = pd.to_numeric(df['收盤價'].str.replace(",",""))
...
def get_data_months(stockNo, months=2, delay=3):
    now = datetime.now()
    data = []
    while months > 0:
...
if __name__=="__main__":
    ...
    df = get_data_months("3008", months=1, delay=3)
    print(df)